...
public class DeviceListActivity extends AppCompatActivity implements DeviceCreateListener {

    private long studentRegNo;

    private DatabaseQueryClass databaseQueryClass = new DatabaseQueryClass(this);

    private List<Device> subjectList = new ArrayList<>();

    private TextView summaryTextView;
    private TextView subjectListEmptyTextView;
    private RecyclerView recyclerView;
    private DeviceListRecyclerViewAdapter subjectListRecyclerViewAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        studentRegNo = getIntent().getLongExtra(Config.ROOM_REGISTRATION, -1);

        subjectList.addAll(databaseQueryClass.getAllSubjectsByRegNo(studentRegNo));

        subjectListRecyclerViewAdapter = new DeviceListRecyclerViewAdapter(this, subjectList);
    ...
    private void openSubjectCreateDialog() {
        DeviceCreateDialogFragment subjectCreateDialogFragment = DeviceCreateDialogFragment.newInstance(studentRegNo, this);
        subjectCreateDialogFragment.show(getSupportFragmentManager(), Config.CREATE_DEVICE);
    }

    @Override
    public void onSubjectCreated(Device subject) {
